...
public class Connector {
    ...
    /**
     * Set the current DataFormatters server to live, clone, or local, or throw
     * an error;
     *
     * @param server
     */
	...
	default:
                throw new IllegalArgumentException("Invalid server option. Please use one of the following options: ['live', 'clone', 'sockets', 'local'].");
        }
    }
    ...
    /**
     * This reformats the coordinates in Earthwuake tweet such that there will
     * be no arrays present when casting to the JSONObject. for the resons
     * described below Mihai
     *
     * @param text
     * @return
     */
    ...
    /**
     * Find the pattern of the coordinates (as an array) and replace them with
     * {"lat": value, "long":value} This is done since it is difficult to cast
     * an JSONArray to a JSON object containing arrays Mihai
     *
     * @param text
     * @return
     */
     ...
     * method with a hash table for different patters Mihai
     *
     * @param text
     * @return
     */
    ...
    /**
     * Traverse JSON in a type-safe manner. This is somewhat complicated, but
     * comes with the advantage that the debugging reports are far clearer when
     * you know the whole path you are searching for.
     *
     * Use this syntax: [ number ] to access an array element, ["quoted string"]
     * to get an object attribute
     *
     * @param sequence
     * @param o
     */
    ...
    /**
     * Execute an Apache Fluent Request. Decorates HTTP error tracebacks with
     * urls and server {"error": "..."} responses. Throws an IOException with
     * URL if the server returns an empty response. Returns server response if
     * the status code is >= 400 but can still throw other exceptions if JSON
     * parsing fails when formatting server JSON response
     *
     * @param request
    ...
    /**
     * Execute a simple GET request relative to the server root. Omit the
     * leading http://hostname, but include the leading /: [good]:
     * /api/followgraph/user/sean [bad]: api/followgraph/user/sean [bad]:
     * http://myserver:9183/api/followgraph/user/sean   NullPointerException
     */
    ...
    /**
     * Execute a request of earthquakes to
     * https://earthquakes-uncc.herokuapp.com/eq/
     *
     * @param url
     * @return JSON string
     * @throws RateLimitException
