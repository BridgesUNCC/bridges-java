...
/**
 * @brief The Bridges class is the main class that provides interfaces to
 * datasets, maintains user and assignment information, and connects to the
 * Bridges server.
 *
 * The Bridges class is responsible for initializing the Bridges system,
 * specifying parameters (user id, assignment id, title, description, data
 * structure type, etc) for the student assignment, generating the data
 * structure representation and transmission to the Bridges server. In addition,
 * it provides interfaces to a number of real-world datasets, that makes it easy
 * to access the data for use algorithms/data structure assignments. <br>
 *
 * <b>Datasets.</b> The datasets that are currently supported through the
 * BRIDGES API include USGS Earthquake Data, IMDB Actor/Movie Data (2 versions),
 * Gutenberg Book Collection Meta Data, a Video Game Dataset and Shakespeare
 * Dataset. More information is found in the respective methods (below) and at
 * <p>
 * http://bridgesuncc.github.io/datasets.html
 * <p>
 *
 * A typical Bridges program includes creating the Bridges object, followed by
 * creation of the data structure by the user, assigning visual attributes to
 * elements of the data structure, followed by specification of teh data
 * structure type and the call to visualize the data structure
 * (Bridges::setDataStructure() and visualize() methods).
 *
 * @author Sean Gallagher, Kalpathi Subramanaian, Mihai Mehedint, David
 * Burlinson.
 *
...
public class Bridges {
     ...
     * @param appl_id this is the appl authentication key(from the Bridges
     * account)
    ...
    /**
     *
     * @param server server to which to connect. Options are: ['live', 'local',
     * 'clone'], and 'live' is the default;
     *
     */
    ...
    /**
     * Turns on map overlay for subsequent visualizations - used with location
     * specific datasets
     *
    ...
    /**
     * @param coord this is the desired coordinate space argument Options are:
     * ['cartesian', 'albersusa', 'equirectangular'], and 'cartesian' is the
     * default;
	 *
     */
	...
	*
    ...
    /**
     * This helper function provides a simple API to retrieve current USGS
     * earthquake Tweet data from the USGS website
     * (https://earthquake.usgs.gov/earthquakes/map/); The data is retrieved and
     * formatted into a list of EarthquakeUSGS objects.
     *
     ...
     * @param maxElements the number of earthquake records retrieved, limited to
     * 5000
    ...
    /**
     * This helper function provides access to a small curated IMDB dataset. The
     * data is retrieved, formatted into a list of ActorMovieIMDB objects (about
     * 1800 pairs).
     *
     ...
     * @throws Exception if the request fails
     *
     * @return a list of ActorMovieIMDB objects, but only actor, movie, movie
     * genre and movie rating are returned.
     */
    ...
    /**
     * This helper function provides access to a second curated IMDB dataset;
     * the data is retrieved, formatted into a list of ActorMovieIMDB objects
     *
     * This version of the IMDB Actor/Movie data contains for each record, actor
     * name, movie name, movie genres, movie rating; refer to
     * <p>
     * &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;http://bridgesuncc.github.io/datasets.html
     * <p>
     * for more information and to look at the dataset.
    ...
    /**
     * This helper function provides access to the meta-data of the Gutenberg
     * book collection (about 1000 books); the data is retrieved, formatted into
     * a list of GutenbergBook objects.
     *
     * Each book in this collection has for each record, information on author
     * (name, birth, death), title, languages, genres, subjects, metrics(number
     * of chars, words, sentences, difficult words), url downloads. More
     * information and commands to access the data can be found at
     * <p>
     * &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;http://bridgesuncc.github.io/datasets.html
     * <br>
     * for more information and to look at the dataset.
     ...
     * collection.
     *
     * Each record in this collection has information on game title, platform,
     * rating, and genre. For more information and to look at the data, refer to
     * <p>
     * &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;http://bridgesuncc.github.io/datasets.html
     * <p>
     *
     ...
     * collection.
     *
     * Each record in this collection has information on song title, artist,
     * album, year, lyrics, and genre. For more information and to look at the
     * data, refer to
     * <p>
     ...
     * lyrics, and genre. For more information and to look at the data, refer to
     * <p>
     * &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;https://bridgesdata.herokuapp.com/api/datasets/songs
     * <p>
     *
    ...
    /**
     * This helper function provides access to a collection of Shakespeare
     * plays, poems and plays.
     *
     * Each record in this collection has information on title, type (poem,
     * Sonnet, play) and text. <br>
     *
     ...
     * @throws Exception if the request fails
     *
     * @param {String} [works] Valid input: "Plays" or "Poems". The works flag
     * is optional.
     *
     * @param {Boolean} [textOnly] True returns only words, no punctuation. The
     * textOnly flag is optional.
     *
     ...
     * https://www.cdc.gov/cancer/npcr/uscs/download_data.htm and curated by
     * Austin (Cory) Bart as part of the Corgis data collection
    ...
    /**
     *
     * This method sets the handle to the current data structure; this can be an
     * array, the head of a linked list, root of a tree structure, a graph
     * Arrays of upto 3 dimensions are suppported. It can be any of the data
     * structures supported by BRIDGES. Polymorphism and type casting is used to
     * determine the actual data structure and extract its representtion.
     *
    ...
    /**
     *
     * This method generates the representation of the current data structure
     * (JSON) and sends that to the Bridges server for generating a
     * visualization.
     *
