...
public class DataFormatter {
    ...
    /**
     * Idiom for enabling ordered iteration on any map. The reason for this is
     * to make the strings compare equal for testing
     *
     * @param values
     * @return
     */
    ...
    /**
     * Automatically choose whether to open a node identifier with: Twitter via
     * followers(), TMDb with movies(), or RottenTomatoes with actors()
     *
     * @param identifier
     * @param max
    ...
    /**
     * List the user's followers as more FollowGraphNodes. Limit the result to
     * `max` followers. Note that results are batched, so a large `max` (as high
     * as 200) _may_ only count as one request. See DataFormatters.followgraph()
    ...
    /**
     * List the user's tweets in the current twitter account. Limit the result
     * to `max` followers. Note that results are batched, so a large `max` (as
     * high as 500) _may_ only count as one request. See
     * DataFormatters.followgraph() for more about rate limiting.
     *
     * @throws MyExceptionClass
     * @throws IOException
     */
...
//			System.out.println("allUSGS: " + allUSGS.get(0));
    ...
    /**
     * The next(List<Tweet>, int) method retrieves the next batch of tweets and
     * adds deep copy of those tweets to the current list
     *
     ...
     *
     * This resource has unlimited queries but has caveats. Not every extra that
     * played in every movie ever is listed in the database and some movies are
     * documented rather sparsely. Expect some to be missing.
     *
     * @throws IOException
     * @throws RateLimitException
     */
    ...
    /**
     * Get USGS earthquake data USGS Tweet data
     * (https://earthquake.usgs.gov/earthquakes/map/) retrieved, formatted into
     * a list of EarthquakeUSGS objects
     *
    ...
    public static ArrayList<EarthquakeUSGS> getEarthquakeUSGSData(
        ...
        if (status == 200) {
            ...
            for (int i = 0; i < json.size(); i++) {
                ...
                JSONArray coords = (JSONArray) ((JSONObject) item.get("geometry")).get("coordinates");
    ...
    /**
     * Get ActorMovie IMDB Data retrieved, formatted into a list of
     ...
     * @throws Exception if the request fails
     *
     * @return a list of ActorMovieIMDB objects, consisting of actor name, movie
     * name, movie genre and movie rating is returned.
     *
     */
    ...
    /**
     *
     * Get meta data of the Gutenberg book collection (1000 books) This function
     * retrieves, and formats the data into a list of GutenbergBook objects
     *
     * @throws Exception if the request fails
     *
     ...
     *
     * This function retrieves and formats the data into a list of Song objects.
     * This version of the API retrieves all the cached songs in the local DB.
     *
     * @throws Exception if the request fails
     *
     ...
     *
     * This function retrieves and formats the data into a Song object. The song
     * if not cached in the local DB is queried and added to the DB
     *
     * @throws Exception if the request fails
     *
     ...
     *
     * This function retrieves and formats the data into a a list of Shakespeare
     * objects.
     *
     * @throws Exception if the request fails
     *
     * @param works can be either "plays" or "poems". If this is specified, then
     * only these types of works are retrieved.
    ...
    public static ArrayList<CancerIncidence> getCancerIncidenceData() throws Exception {
        ...
        if (status == 200) {
            ...
            for (int i = 0; i < 10; i++) {
                ...
                c.setAgeAdjustedRate(((Number) age.get("Age Adjusted Rate")).doubleValue());
                c.setAgeAdjustedCI_Lower(((Number) age.get("Age Adjusted CI Lower")).doubleValue());
                c.setAgeAdjustedCI_Upper(((Number) age.get("Age Adjusted CI Upper")).doubleValue());
